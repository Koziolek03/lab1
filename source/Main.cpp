...
class TexturedAsteroid : public Asteroid {
	public:
		TexturedAsteroid(int w, int h) : Asteroid(w, h) {
			baseDamage = 12;
			texture = LoadTexture("kurwinox.png");
			if (texture.id == 0) {
				// Fallback if texture fails to load
				Image img = GenImageChecked(64, 64, 16, 16, RED, BLUE);
...
// Shape selector
enum class AsteroidShape { TRIANGLE = 3, SQUARE = 4, PENTAGON = 5, TEXTURED = 6, RANDOM = 0 };
...
static inline std::unique_ptr<Asteroid> MakeAsteroid(int w, int h, AsteroidShape shape) {
    switch (shape) {
        ...
        case AsteroidShape::TEXTURED:
            return std::make_unique<TexturedAsteroid>(w, h);
        default:
	...
	void Run() {
		...
		while (!WindowShouldClose()) {
			...
			if (IsKeyPressed(KEY_FIVE)) {
				currentShape = AsteroidShape::TEXTURED;
			}
